//============================================================================
// Name        : LUOpenCL.cpp
// Author      : Yensy Helena Gomez, Lina Maria Perez, John Haiber Osorio,
// Version     :
// Copyright   :
// Description : Solve Newton Cooling Law equation using OpenCL, Ansi-style
//============================================================================
...
int prepararOpenCL(float *a, int n, int *pivot, float *b, float *x, float *y)
{
	...
	cl_kernel permutarVectorKernel;
	cl_kernel solveSystemEquationsY;
	cl_kernel solveSystemEquationsX;
...
/////////////////////CREATE BUFFERS HERE//////////////////////////////
	...
	cl_mem a_Kernel, rowMax_Kernel, pivot_Kernel, b_Kernel, x_Kernel, y_Kernel; // Input buffers on device
...
//////////////////////////Create a buffer for matrix a//////////////////////////////////////////////////////
	...
	/////////////////////////Create a buffer for b_Kernel///////////////////////////////////////////////////////////
	b_Kernel = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(cl_float) * n, b, &status);
	if (status != CL_SUCCESS || b_Kernel == NULL) {
		printf("clCreateBuffer failed b_Kernel\n");
		exit(-1);
	}
	/////////////////////////Create a buffer for x_Kernel///////////////////////////////////////////////////////////
	x_Kernel = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_float) * n, NULL, &status);
	if (status != CL_SUCCESS || x_Kernel == NULL) {
		printf("clCreateBuffer failed x_Kernel\n");
		exit(-1);
	}
	/////////////////////////Create a buffer for y_Kernel///////////////////////////////////////////////////////////
	y_Kernel = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_float) * n, NULL, &status);
	if (status != CL_SUCCESS || y_Kernel == NULL) {
		printf("clCreateBuffer failed y_Kernel\n");
		exit(-1);
	}
...
///////////////////////CREATE KERNEL/////////////////////////////////////////
	...
	permutarVectorKernel = clCreateKernel(program,"permutarVectorKernel", &status);
	if (status != CL_SUCCESS) {
		printf("clCreateKernel failed permutarVectorKernel\n");
		exit(-1);
	}
	
	solveSystemEquationsY = clCreateKernel(program,"solveSystemEquationsY", &status);
	if (status != CL_SUCCESS) {
		printf("clCreateKernel failed solveSystemEquationsY\n");
		exit(-1);
	}
	solveSystemEquationsX = clCreateKernel(program,"solveSystemEquationsX", &status);
	if (status != CL_SUCCESS) {
		printf("clCreateKernel failed solveSystemEquationsX\n");
		exit(-1);
	}
	...
	status = clSetKernelArg(permutarVectorKernel, 0,sizeof(cl_mem), &b_Kernel);
	if (status != CL_SUCCESS) {
		printf("clSetKernelArg b_Kernel failed permutarVectorKernel\n");
		exit(-1);
	}

	status = clSetKernelArg(permutarVectorKernel, 1,sizeof(cl_mem), &pivot_Kernel);
	if (status != CL_SUCCESS) {
		printf("clSetKernelArg pivot_Kernel failed permutarVectorKernel\n");
		exit(-1);
	}

	status = clSetKernelArg(solveSystemEquationsY, 0,sizeof(cl_mem), &a_Kernel);
	if (status != CL_SUCCESS) {
		printf("clSetKernelArg a_Kernel failed solveSystemEquationsY\n");
		exit(-1);
	}

	status = clSetKernelArg(solveSystemEquationsY, 1,sizeof(cl_mem), &b_Kernel);
	if (status != CL_SUCCESS) {
		printf("clSetKernelArg b_Kernel failed solveSystemEquationsY\n");
		exit(-1);
	}


	status = clSetKernelArg(solveSystemEquationsY, 2,sizeof(cl_mem), &y_Kernel);
	if (status != CL_SUCCESS) {
		printf("clSetKernelArg y_Kernel failed solveSystemEquationsY\n");
		exit(-1);
	}



	status = clSetKernelArg(solveSystemEquationsX, 0,sizeof(cl_mem), &a_Kernel);
	if (status != CL_SUCCESS) {
		printf("clSetKernelArg a_Kernel failed solveSystemEquationsX\n");
		exit(-1);
	}


	status = clSetKernelArg(solveSystemEquationsX, 1,sizeof(cl_mem), &x_Kernel);
	if (status != CL_SUCCESS) {
		printf("clSetKernelArg x_Kernel failed solveSystemEquationsX\n");
		exit(-1);
	}

	status = clSetKernelArg(solveSystemEquationsX, 2,sizeof(cl_mem), &y_Kernel);
	if (status != CL_SUCCESS) {
		printf("clSetKernelArg y_Kernel failed solveSystemEquationsX\n");
		exit(-1);
	}
...
/////////////////////////CALL KERNEL calculo alpha's //////////////////////////////////////////
	...
	}////Termina el cálculo de la Matriz LU.

	//////////////////////////////Inicia la Solucion del sistema de ecuaciones////////////////////////////////
	global[0] = n;
	status = clEnqueueNDRangeKernel(cmdQueue,permutarVectorKernel, 1, NULL, global, NULL, 0,NULL, NULL);
	if (status != CL_SUCCESS) {
		printf("clEnqueueNDRangeKernel permutarVectorKernel failed\n");
		exit(-1);
	}
	global[0] = n;
	status = clEnqueueNDRangeKernel(cmdQueue,solveSystemEquationsY, 1, NULL, global, NULL, 0,NULL, NULL);
	if (status != CL_SUCCESS) {
		printf("clEnqueueNDRangeKernel solveSystemEquationsY failed\n");
		exit(-1);
	}

	global[0] = n;
	status = clEnqueueNDRangeKernel(cmdQueue,solveSystemEquationsX, 1, NULL, global, NULL, 0,NULL, NULL);
	if (status != CL_SUCCESS) {
		printf("clEnqueueNDRangeKernel solveSystemEquationsX failed\n");
		exit(-1);
	}
	////////////////////////////////Termina la solucion del sistema de ecuaciones//////////////////////////
	
	//clEnqueueReadBuffer(cmdQueue, a_Kernel, CL_TRUE, 0,sizeof(float)*n*n, a, 0, NULL, NULL);
	//clEnqueueReadBuffer(cmdQueue, pivot_Kernel, CL_TRUE, 0,sizeof(float)*n, pivot, 0, NULL, NULL);
	clEnqueueReadBuffer(cmdQueue, x_Kernel, CL_TRUE, 0,sizeof(float)*n, x, 0, NULL, NULL);
	//clEnqueueReadBuffer(cmdQueue, b_Kernel, CL_TRUE, 0,sizeof(float)*n, b, 0, NULL, NULL);
	//clEnqueueReadBuffer(cmdQueue, y_Kernel, CL_TRUE, 0,sizeof(float)*n, y, 0, NULL, NULL);
...
////////////////////////////////////////////////////////////////////////////
	...
	clReleaseMemObject(rowMax_Kernel);
	clReleaseMemObject(pivot_Kernel);
	clReleaseMemObject(b_Kernel);
	clReleaseMemObject(x_Kernel);
	clReleaseMemObject(y_Kernel);
...
int imprimirMatrizLUArchivo(int numeroEcuaciones,float *a){
	...
	for(i = 0; i < numeroEcuaciones; i++){
		for(j = 0;j < numeroEcuaciones ; j++)
			fprintf(pFile, "%.3f  ", a[i*numeroEcuaciones+j]);
		fprintf(pFile,"\n");
	}
	fclose(pFile);
}
...
int imprimirVector(int numeroEcuaciones,float *v){
	int i;
	FILE *pFile;
	pFile = fopen("salidaVector", "w");
	for(i = 0; i < numeroEcuaciones; i++){		
		fprintf(pFile, "%.3f\n", v[i]);		
	}
	fclose(pFile);
...
int main(int argc, char *argv[]){
	...
	y = (float *)malloc(sizeof(float)*numeroEcuaciones);
	...
	start = clock();
	prepararOpenCL(a,numeroEcuaciones,pivot,b,x,y);
	end = clock();
...
/*	permutarVector(b,pivot,numeroEcuaciones);
	...
	//imprimirVector(numeroEcuaciones,y);
